...
GanttEntry ganttChart[1010];

void printGanttChart() {
    int i, j;
    printf("Gantt Chart:\n");
    printf(" ");
    for (i = 0; i < ganttChartSize; i++) {
        for (j = ganttChart[i].finish_time - ganttChart[i].start_time; j > 0; j--) {
            printf("--");
        }
        printf(" ");
    }
    ...
    for (i = 0; i < ganttChartSize; i++) {
        ...
        if (ganttChart[i].finish_time > 9) {
            //printf(" ");
        }
        printf("%d", ganttChart[i].finish_time);
    }
    printf("\n");
}
...
void simulate(int amount, int alg, int preemptive, int time_quantum) { 
	...
	if(prevProcess != runningProcess) { 
		if (prevProcess != NULL) {
            ganttChart[ganttChartSize].pid = prevProcess->pid;
    ...
    if(runningProcess != NULL) { 
        ...
        if(runningProcess->CPUremainingTime <= 0) { 
			ganttChart[ganttChartSize].pid = runningProcess->pid;
            ganttChart[ganttChartSize].start_time = amount - timeConsumed + 1;
            ganttChart[ganttChartSize].finish_time = amount + 1;
            ganttChartSize++;
